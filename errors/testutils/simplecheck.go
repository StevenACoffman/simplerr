...
// AssertWithf calls t.Fatal if the condition is false. Also
// the text of the assertion is printed.
func (t *T) AssertWithf(cond bool, format string) {
	t.Helper()
	if !cond {
		t.failWithf(true, format)
	}
}
...
func (t *T) Check(cond bool, message string) {
	t.Helper()
	if !cond {
		t.failWithf(false, "check failed:"+message)
	}
}
